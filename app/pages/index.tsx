...
const Home: NextPage = () => {
    ...
    // ** Why is the default state "0" and not `zero` here?
    const [removeLPTokens, setRemoveLPTokens] = useState("0");

    /** Variables to keep track of swap functionality */
    // Amount that the user wants to swap
    // The swapAmount unit depends on that the user clicks (ETH or CD)
    ...
    const getAmounts = async () => {
        try {
            ...
            // Fetching address of the connected user
            // ** How does Kevin deal w `Property 'getAddress' does not exist on type 'Web3Provider | JsonRpcSigner'.` TS errors?
            // @ts-ignore
    ...
    const _swapTokens = async () => {
        try {
            ...
            // ** What if user enters a negative number?
    ...
    const _getAmountOfTokensReceivedFromSwap = async (_swapAmount: number) => {
        try {
            // Convert the amount entered by the user to a BigNumber using the `parseEther` library from `ethers.js`
            // ** What if `_swapAmount` is negative?
    ...
    const _addLiquidity = async () => {
        try {
            ...
            if (!addCDTokens.eq(zero) && !addEtherWei.eq(zero)) {
                ...
                // ** What about reinitializing ETH as well?
                setAddCDTokens(zero);
    ...
    const _getTokensAfterRemove = async (_removeLPTokens: string) => {
        try {
            ...
            // ** Why does `tokensAfterRemove` has `undefined` as one of the types? A: because of try/catch block. Removed it :(
            const tokensAfterRemove = await getTokensAfterRemove(
                provider,
                removeLPTokenWei,
            ...
            setRemoveCD(tokensAfterRemove!._removeCD);
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const getProviderOrSigner = async (needSigner = false) => {
        ...
        // ** How does Kevin deal with `Object is possibly 'undefined'` TS errors?
        // @ts-ignore
                ...
                <div>
                    <input
                        ...
                        onChange={async (e) => {
                            ...
                            // ** Is this the correct solution from `await _getAmountOfTokensReceivedFromSwap(e.target.value || "0")`?
                            await _getAmountOfTokensReceivedFromSwap(parseInt(e.target.value) || 0);
                        }}
                        className="input"
                        value={swapAmount}
                    />
    ...
    return (
        <div>
            ...
            <div className="main">
                ...
                <div>
                    <img className="image" src="./cryptodev.svg" />
                </div>
            </div>
