...
// Our contract inherits `ERC20` because we'll need to mint additional ERC20s (LP tokens)
contract Exchange is ERC20 {
    address public cryptoDevTokenAddress;

    // Exchange is inheriting ERC20, because our exchange would keep track of Crypto Dev LP tokens
    constructor(address _CryptoDevtoken) ERC20("CryptoDev LP Token", "CDLP") {
        // `address(0)` returns a null address by default
    ...
    function getReserve() public view returns (uint256) {
        // `balanceOf` -> func inherited from ERC20 contract
    ...
    function addLiquidity(uint256 _amount) public payable returns (uint256) {
        ...
        // ** Where is the user supplying `Eth` here (i.e. `msg.value`)?
        /*
    ...
    */
        if (cryptoDevTokenReserve == 0) {
            ...
            // `transferFrom` -> ERC20.sol func
            cryptoDevToken.transferFrom(msg.sender, address(this), _amount);
            // ** What if there is no `ethBalance` yet when a user is depositing `cryptoDevToken`?
        ...
        */
            ...
            // So doing some math, (cryptoDevTokenAmount user can add) = (Eth Sent by the user * cryptoDevTokenReserve /Eth Reserve);
            uint256 cryptoDevTokenAmount = (msg.value * cryptoDevTokenReserve) / (ethReserve);
            // ** Wait, you can send more than the minimum? How does that work?
            ...
            // with some math -> liquidity =  (totalSupply of LP tokens in contract * (Eth sent by the user))/(Eth reserve in the contract)
    ...
    function removeLiquidity(uint256 _amount) public returns (uint256, uint256) {
        ...
        // ** Does `_totalSupply` represent the total supply of CD tokens? How?
        ...
        // remove liquidity
        // ** How are we ensuring that we're burning the LP token, and not any other ERC20 we hold in our accout/address?
        _burn(msg.sender, _amount);
        // Transfer `ethAmount` of Eth from the contract to the user's wallet
        payable(msg.sender).transfer(ethAmount); // Special syntax for native L1 token (ETH)
        // Transfer `cryptoDevTokenAmount` of Crypto Dev tokens from the contract to the user's wallet
        // ** Wait, we're sending the CD tokens from the cryptoDevToken contract & not the current `Exchange` contract? Why?
    ...
    function getAmountOfTokens(
        ...
        // ** What about adding `inputAmount` to this `require` condition as well?
        ...
        // ** Don't understand these equations
    ...
    function ethToCryptoDevToken(uint256 _minTokens) public payable {
        ...
        // ** Wait, we're sending the CD tokens from the cryptoDevToken contract & not the current `Exchange` contract? Why?
        // ** Isn't ERC20.sol's `transfer` sending funds from `msg.sender` by default? Then it'd mean we're sending funds to ourselves
    ...
    function cryptoDevTokenToEth(uint256 _tokensSold, uint256 _minEth) public {
        ...
        // ** Analyze ERC20.sol's `transferFrom` more later
